...
class GoodsCateModel extends Model{
	...
	public function getSecList($condition = array(), $field = 'id,name,unit,purchasing_price,purchasing_point,img_1,img_2'){
		$condition['in_status'] = 1;
		$condition['parent_id'] = 0;
		$condition['delete_time'] = 0;
		$return = [];
		$parent = $this->where($condition)->field('id')->select();
		if($parent){
			foreach($parent as $v){
				$condition['parent_id'] = $v['id'];	
				$secGoods = $this->where($condition)->field($field)->select();	
				if($secGoods){
